...
- 实现方式：
  - **CLI模式（推荐，默认）**：通过 SSH 执行远程包装脚本，使用 pexpect 控制交互式 `llamafactory-cli chat` 命令
  - **Script模式（备选）**：通过 SSH 执行 Python 脚本，使用 LlamaFactory 的 Python API 进行推理
- 配置支持：
  - 工作目录设置（确保在正确目录下执行）
  - Conda 环境支持（如果 llamafactory-cli 在 conda 环境中）
  - 环境变量自动加载（使用 bash -l 加载完整的用户环境）
- JSON 参数通过 base64 编码传递，避免 shell 解析问题
- 自动过滤系统日志和警告信息（如 FP32 警告），只返回纯 Assistant 回复
...
- **对话功能**: pexpect（控制交互式CLI程序，CLI模式）+ base64（安全传递JSON参数）
...
class SSHService:
    ...
    def execute_chat_script(self, config: Dict, script_path: str, timeout: int = 300) -> Dict:
        ...
        """
        # 将配置序列化为 JSON，并使用 base64 编码
        import base64
        config_json = json.dumps(config, ensure_ascii=False)
        config_b64 = base64.b64encode(config_json.encode('utf-8')).decode('ascii')
        
        # 构建命令（通过命令行参数传递 base64 编码的 JSON）
        work_dir = settings.remote_work_dir
        command = f"cd {work_dir} && python3 {script_path} \"{config_b64}\""
    ...
    def execute_chat_cli(self, config: Dict, template: str = "qwen2", timeout: int = 300) -> Dict:
        """
        通过交互式CLI执行对话推理（CLI模式，使用pexpect在远程服务器上）
        参数:
        ...
        # 构建完整命令，包括工作目录切换、conda环境激活等
        wrapper_script = settings.chat_script_path
        ...
        cmd_parts = [f"cd {work_dir}"]
        if conda_env:
            cmd_parts.append("source ~/.bashrc 2>/dev/null || true")
            cmd_parts.append("source $(conda info --base)/etc/profile.d/conda.sh 2>/dev/null || true")
            cmd_parts.append(f"conda activate {conda_env} 2>/dev/null || true")
...
#### 3.3.5 对话推理执行
**功能描述**: 通过 SSH 执行模型推理（支持 CLI 模式和 Script 模式）

**实现方式 - CLI模式（推荐）**:
- CLI模式使用交互式的 `llamafactory-cli chat` 命令
- 后端通过 SSH 执行远程包装脚本 `chat_cli_wrapper.py`
- 包装脚本使用 pexpect 库与交互式 CLI 程序交互：
  - 启动 `llamafactory-cli chat` 命令
  - 等待欢迎消息和提示符
  - 发送用户消息
  - 等待并提取 Assistant 回复
- JSON 配置通过 base64 编码传递，避免 shell 解析问题
- 自动过滤系统日志和警告信息（如 `[WARNING|logging.py:328]` 和 `FP32)` 警告）
- 执行环境：
  - 自动切换到配置的工作目录（`REMOTE_WORK_DIR`）
  - 支持 conda 环境激活（如果配置了 `CONDA_ENV`）
  - 使用 `bash -l` 加载完整的用户环境（.bashrc, .bash_profile 等）

**包装脚本部署**:
- 脚本文件：`backend/chat_cli_wrapper.py`
- 需要部署到远程服务器的固定路径（配置为 `CHAT_SCRIPT_PATH`）
- 远程服务器需要安装 pexpect：`pip install pexpect`
- 确保脚本有执行权限：`chmod +x chat_cli_wrapper.py`

**配置要求**:
```env
# .env 配置示例
CHAT_MODE=cli  # 使用CLI模式
CHAT_SCRIPT_PATH=/path/to/llamafactory/chat_cli_wrapper.py  # 包装脚本路径
REMOTE_WORK_DIR=/root/LLaMA-Factory  # 工作目录
CONDA_ENV=base  # Conda环境（可选，如果llamafactory-cli在conda环境中）
CHAT_TIMEOUT=300  # 超时时间（秒）
```
...
bash -l -c 'cd /root/LLaMA-Factory && conda activate base && python3 chat_cli_wrapper.py "<base64_encoded_json>"'
```

**实现方式 - Script模式（备选）**:
- Script模式使用 LlamaFactory 的 Python API
- 通过 SSH 执行 Python 脚本，脚本使用 `ChatModel` 类进行推理
...
def main():
    # 从命令行参数读取配置（base64编码）
    if len(sys.argv) > 1:
        config_str = base64.b64decode(sys.argv[1].encode('ascii')).decode('utf-8')
...
**实现注意事项**:
1. **脚本位置**: 
   - CLI模式：包装脚本需要部署到 `CHAT_SCRIPT_PATH` 指定的路径
   - Script模式：Python 推理脚本需要部署到指定路径
2. **环境要求**: 
   - CLI模式：远程服务器需要安装 pexpect 和 llamafactory-cli
   - Script模式：需要安装 LlamaFactory Python 库
3. **工作目录**: 确保 `REMOTE_WORK_DIR` 设置为 LlamaFactory 安装目录
4. **超时设置**: 设置合理的 SSH 命令执行超时时间（默认 300 秒），避免长时间等待
5. **错误处理**: 脚本执行失败时，捕获错误信息并返回给前端
6. **日志过滤**: CLI模式会自动过滤系统日志，只返回纯 Assistant 回复
7. **JSON传递**: 使用 base64 编码避免 shell 解析问题，确保特殊字符正确传递

**性能考虑**:
- 每次对话都需要建立 SSH 连接和执行脚本，响应时间较长（可能数秒到数十秒）
- 不适合高并发场景（最小实现可接受）
- 模型加载时间可能较长（首次调用或模型切换时，CLI模式每次都会重新加载模型）
- 如果频繁调用，考虑在远程服务器上使用模型服务（不在最小实现范围内）
...
# 对话推理配置
CHAT_MODE=cli  # 使用CLI模式（cli）或Script模式（script）
CHAT_SCRIPT_PATH=/path/to/llamafactory/chat_cli_wrapper.py  # 包装脚本路径（CLI模式）或推理脚本路径（Script模式）
CHAT_TIMEOUT=300  # 对话超时时间（秒）
CONDA_ENV=base  # Conda环境名称（可选，如果llamafactory-cli在conda环境中）
LLAMAFACTORY_CLI_PATH=llamafactory-cli  # llamafactory-cli路径（可选，默认在PATH中查找）
```
...
# 2. 将包装脚本上传到远程服务器
# 从本地 backend/ 目录上传 chat_cli_wrapper.py 到远程服务器
scp backend/chat_cli_wrapper.py user@remote:/path/to/llamafactory/chat_cli_wrapper.py

# 3. 确保脚本有执行权限
chmod +x /path/to/llamafactory/chat_cli_wrapper.py

# 4. 测试脚本是否正常工作（可选）
cd /path/to/llamafactory
python3 chat_cli_wrapper.py '{"base_model_path":"/path/to/model","adapter_path":"/path/to/adapter","template":"qwen2","message":"你好"}'

# 注意：
# - 确保 llamafactory-cli 在 PATH 中，或通过 conda 环境激活
# - 脚本会自动过滤系统日志，只返回纯 Assistant 回复
```

**远程服务器准备（对话功能 - Script模式，备选）**:
```bash
# 1. 在远程服务器上创建对话推理脚本 chat_inference.py
# 脚本内容参考 3.3.5 节的 Script 模式示例代码
...
# 4. 测试脚本是否正常工作（可选）
python3 /path/to/llamafactory/chat_inference.py '<base64_encoded_json>'
...
- pexpect 文档: https://pexpect.readthedocs.io/
...
### v2.1 (2025-01-05)
- **对话功能实现更新**：
  - 实现CLI模式：使用pexpect控制交互式`llamafactory-cli chat`命令
  - 添加base64编码传递JSON，避免shell解析问题
  - 添加工作目录和环境变量自动处理
  - 支持conda环境自动激活
  - 实现系统日志和警告信息自动过滤（特别是FP32警告）
  - 支持CLI模式和Script模式两种实现方式
- 添加pexpect依赖到requirements.txt
- 创建chat_cli_wrapper.py包装脚本用于CLI模式
- 更新SSH服务支持chat CLI执行
- 更新配置支持chat_mode、conda_env等新配置项
